...
import { Search, ShoppingCart, Trash2, RefreshCw, CheckCircle, XCircle, Clock, Printer, History } from 'lucide-react';
...
let pedidosCurrency = 'ARS';
const formatMoney = (n: number) => fmtCurrency(n, pedidosCurrency);
...
<div class="page">
  ...
  <div class="footer">Documento generado por Nexus Full &mdash; ${new Date().toLocaleString()}</div>
</div>
...
export default function PedidosWeb({ onRefresh, currency }: PedidosWebProps) {
  pedidosCurrency = currency || 'ARS';
  ...
  const load = async () => {
    setLoading(true);
    try {
      const localOrders = await fetchOrders().catch(() => [] as Order[]);
      localOrders.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  ...
  const updateStatus = async (id: string, status: string) => {
    try {
      ...
      alert('No se pudo actualizar el pedido ' + id + '. Verifique la conexión.');
    }
    load();
    onRefresh();
  };
  ...
  const remove = async (id: string) => {
    ...
    try {
      ...
      alert('No se pudo eliminar el pedido ' + id + '. Verifique la conexión.');
    }
    load();
    onRefresh();
  };
